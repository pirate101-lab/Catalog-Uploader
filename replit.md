...
- **`artifacts/api-server`** — Express API for storefront (`src/routes/storefront.ts`) and checkout stubs (`src/routes/checkout.ts`). Reads from `data/catalog_lite.json` (18,302 women's products) and `data/catalog_men_lite.json` (2,361 men's products) via `src/lib/catalog.ts`, which merges them and tags each row with `gender: "women" | "men"`. Men's IDs are namespaced with an `m-` prefix to avoid collisions. Service binds `PORT=8080`, paths `/api`. Storefront endpoints accept `?gender=men|women`.
  - **Synthesised merch buckets** (women only): `src/lib/catalog.ts` derives four overlapping bucket flags at boot — `isNewIn` (top 30% by numeric id), `isCollection` (~50% via FNV hash), `isTikTokVerified` (~25%), `isTrending` (top 30% by `trendScore` = hash + mid-tier price boost). Counts: 5,490 / 9,210 / 4,669 / 5,490. Filterable via `GET /api/storefront/products?bucket=new_in|collection|tiktok_verified|trending`. Men's products always have empty buckets (the four nav tabs are women-only). The synthesis is deterministic: same id + price ⇒ same flags across restarts.
  - **Daily-rotated featured grid**: `?dailyRotate=true&seed=YYYY-MM-DD` Fisher–Yates-shuffles the filtered rows with a Mulberry32 PRNG seeded from the date, so every visitor sees the same lineup until midnight UTC. The homepage passes today's UTC date as `seed` and uses `bucket=tiktok_verified&limit=40`. The `seed` is regex-validated (`^[A-Za-z0-9_-]{1,32}$`) to prevent cache-key flooding.
  - **Why bucket data is synthesised, not scraped**: The Trendsi extractor at `scripts/extract-women.py` (used in Task #20) requires fresh `Access-Token` / `SHOPID` / `TOKEN` cookies. Those cookies are now expired — a probe with the in-script values returns HTTP 200 but `total: 0` items, exactly the script's "refresh credentials" sentinel. Until fresh cookies are captured from a logged-in Trendsi session, the four merch tabs use deterministic synthesis on the existing 18,302-product catalog. Every bucket still surfaces real products with real R2 images and real prices — the only thing missing is upstream "is_new_in" / "is_tiktok_verified" ground truth.
